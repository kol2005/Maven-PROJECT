...
public class IolistService {
	...
	// 아래와 같이 생성자가 여러개 일때
	// 각 생성자 마다 필드변수를 초기화 하는 코드를 넣어주어야하는데
	// 간혹 생략을 했을때 NullPointException이 발생할수 있다
	// IolistService is = new IolistService()
	// IolistService is = new IolistService("aaaa");
	
	// 이럴때 
	// 공통으로 초기화, 생성하는 코드들을 인스턴스 생성자에
	// 작성해주면 생성자 method에 관계없이 자동으로 실행하도록 할 수 있다
	{
		
		ioDao = DBConnectionSEC.getInstance()
				.openSession(true).getMapper(IolistDao.class);
